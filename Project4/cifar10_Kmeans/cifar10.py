...
def create_samples(samples,shape):
...
def _variable_with_Kmeans(name, shape,the_images,wd):

  print('start kmeans')

  samples=create_samples(the_images,shape)
  ...
  # while (tf.not_equal(tf.size(ifEqual),tf.constant(0))) and iter<50:
  while iter<100:
      print(iter)
  ...
  with tf.device('/cpu:0'):
    dtype = tf.float16 if FLAGS.use_fp16 else tf.float32
    var = tf.reshape(updated_centroids,[shape[3],shape[0],shape[1],shape[2]])
    var = tf.transpose(var,[1,2,3,0],name='weights')
...
def inference(images):
  ...
  with tf.variable_scope('conv1') as scope:
    kernel =  _variable_with_Kmeans('weights',
                                    shape=[5, 5, 3, 6],
                                    the_images=images,
                                    wd=0.0)
