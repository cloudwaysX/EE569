...
void ClassifyD2(double*** data, vector<bool> labelMask, vector<int>& labels,vector<vector<double>> centerVec, int dataSizeH, int dataSizeW);
void Kmeans2(double*** data, vector<bool> labelMask, vector<vector<double>> centerVec,vector<int>& labels, int dataSizeH, int dataSizeW);
...

{
	...
	if (argc < 4){
		...
		cout << "program_name output_file clusterNum(total+secondSteps)" << endl;
		return 0;
	}
...
////////////////Phase 1///////////////////////////////////////////////////
	...
	vector<std::vector<double>> centroids1;
	vector<double> theCentroid0;
	for(int n=0;n<featureNum;n++){
		theCentroid0.push_back(mean[0][n]);
	}
	centroids1.push_back(theCentroid0);
	set<pair<int,int>> visited;
	for(int k=1;k<clusterNum/clusterNum2+1;k++){
		double maxD=0;
		int maxD_I[]={0,0};
		for(int i=0;i<SizeH;i++){
			for(int j=0;j<SizeW;j++){
				double D=0;
				for(int n=0;n<featureNum;n++){
					D+=pow(textures_window[i][j][n]-centroids1[k-1][n],2);
				}
		...
		centroids1.push_back(theCentroid);
	}
	centroids1.erase(centroids1.begin());

	vector<int> labels1;
	vector<double> classNum(centroids1.size(),0);
	labels1=Kmeans(textures_window,centroids1, SizeH,SizeW); 

	//////////////////////////////Now the second seg///////////////////////////////
	vector<int> labels2;
	vector<double> classNum2(centroids1.size()*clusterNum2,0);
	vector<vector<double>> centroids2;
	set<pair<int,int>> visited2; 
	for(int i=0;i<centroids1.size();i++){
		centroids2.push_back(centroids1[i]);
		for(int k=1;k<clusterNum2;k++){
			cout<<i<<" "<<k<<endl;
			double maxD=0;
			int maxD_I[]={0,0};
			for(int r=0;r<SizeH;r++){
				for(int c=0;c<SizeW;c++){
					//cout<<i<<" "<<j<<endl;
					if(labels1[r*SizeW+c]!=i) continue;
					double D=0;
					//cout<<i<<" "<<j<<endl;
					for(int n=0;n<featureNum;n++){
						//cout<<"previous one "<<clusterNum2*i+k-1<<endl;
						//cout<<"center size"<<centroids2.size()<<endl;
						D+=pow(textures_window[r][c][n]-centroids2[clusterNum2*i+k-1][n],2);
					}

					if(D>maxD && visited2.find(make_pair(r,c))==visited.end()){
						maxD=D;
						maxD_I[0]=r;maxD_I[1]=c;
						visited.emplace(make_pair(r,c));
					}
				}
			}
			...
			centroids2.push_back(theCentroid);
		}

	}
	//cout<<"labels for phase1: "<<endl();
	for(int i=0;i<SizeW*SizeH;i++){
		labels2.push_back(labels1[i]*clusterNum2);
		//cout<<labels1[i]<<" ";
	}	
	//cout<<endl;
	for(int i=0;i<clusterNum/clusterNum2;i++){
		cout<<"...............Phase2: "<<i<<endl;
		vector<bool> labelMask2(clusterNum,false);
		for(int n=0;n<clusterNum2;n++){
			labelMask2[clusterNum2*i+n]=true;
		}
		Kmeans2(textures_window,labelMask2,centroids2,labels2, SizeH,SizeW); 
	}	
	...
	for(int i =0;i<SizeH;i++){
		for(int j=0;j<SizeW;j++){
			...
			labeldImg[i][j][0]=labels2[i*SizeW+j];
	...
	for(int r=0;r<SizeH;r++){
		...
		for(int c=0;c<SizeW;c++){
			...
			labeldImg=BFS(labeldImg,visitedMap, curP,targetVal,600,SizeH,SizeW);
		}
	}

	/*bool** visitedMap2=new bool*[SizeH];
	int*** labeldImg2=new int**[SizeH];
	for(int i =0;i<SizeH;i++){
		labeldImg2[i]=new int*[SizeW];
		visitedMap2[i]=new bool[SizeW];
	}
	for(int i =0;i<SizeH;i++){
		for(int j=0;j<SizeW;j++){
			labeldImg2[i][j]=new int[BytesPerPixel];
			labeldImg2[i][j][0]=labeldImg[i][j][0];

			visitedMap2[i][j]=false;
		}
	}
		...
		for(int c=0;c<SizeW;c++){
			...
			for(int i=-1;i<=1;i++){
				for(int j=-1;j<=1;j++){
					...
					int newC=c+j;
					if(newR<0||newC<0||newR>=SizeH||newC>=SizeW) continue;
					if(labeldImg2[newR][newC][0]!=labeldImg2[r][c][0]){
						targetVal=labeldImg2[newR][newC][0];
						break;
					}					
				}
			}
			...
			labeldImg2=BFS(labeldImg2,visitedMap2, curP,targetVal,800,SizeH,SizeW);
		}
	}*/
...
void ClassifyD2(double*** data, vector<bool> labelMask, vector<int>& labels,vector<vector<double>> centerVec, int dataSizeH, int dataSizeW){
	for(int r=0;r<dataSizeH;r++){
		for(int c=0;c<dataSizeW;c++){
			int thisCalss=labels[r*dataSizeW+c];
			//cout<<"1"<<endl;
			if(!labelMask[thisCalss]){
				continue;
			}
			//cout<<thisCalss<<"is"<<labelMask[thisCalss]<<endl;
			//cout<<"2"<<endl;
			double minSum=FLT_MAX;
			int minClass=0;
			for(int classI=0;classI<centerVec.size();classI++){
				if(!labelMask[classI]) continue;
				double sum=0;
				for(int n=0;n<centerVec[0].size();n++){
					sum+=pow(centerVec[classI][n]-data[r][c][n],2);
				}
...
void Kmeans2(double*** data, vector<bool> labelMask, vector<vector<double>> centerVec,vector<int>& labels, int dataSizeH, int dataSizeW){
	...
	int k=0;
	while(true){
		ClassifyD2(data, labelMask,labels,centerVec,dataSizeH,dataSizeW);
		if(old == labels){
			cout<<"exit"<<endl;
			break;
		}
		...
		for(int n=0;n<centerVec[0].size();n++){
			for(int r=0;r<dataSizeH;r++){
				for(int c=0;c<dataSizeW;c++){
					int theClass=labels[r*dataSizeW+c];
					if(!labelMask[theClass]) continue;
					new_centerVec[theClass][n]+=data[r][c][n]/classNum[theClass];
				}
			}	
		}
